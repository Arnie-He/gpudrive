...
class Discriminator(nn.Module):
    ...
    def predict_reward(self, states, actions, reward_type="scaled_log"):
        """
        ...
        with torch.no_grad():
            ...
            if reward_type == "scaled_log":
                ...
                # Original GAIL reward: -log(1 - D(s,a))
                reward = -torch.log(1 - prob_expert)
                
            elif reward_type == "wgan":
                # WGAN-style reward: D(s,a) directly
    ...
    def gradient_penalty(self, real_samples, fake_samples, device, lambda_gp=10.0):
        ...
        Args:
            real_samples: Real state-action pairs
            fake_samples: Generated/policy state-action pairs  
        ...
        # Split interpolated samples back into states and actions
        states_interp = interpolated[:, :self.obs_dim]
        actions_interp = interpolated[:, self.obs_dim:]
        
        # Forward pass on interpolated samples
        d_interpolated = self.forward(states_interp, actions_interp)
...
class GAILTrainer:
    ...
    def train_discriminator(self, num_epochs=5):
        ...
        for epoch in range(num_epochs):
            ...
            expert_obs, expert_actions = self.expert_dataset.sample(batch_size)
            ...
            # Handle action concatenation based on action type
            action_space_type = getattr(self.env_config, 'action_type', 'discrete')
            if action_space_type == "continuous":
                all_actions = torch.cat([expert_actions, policy_actions])
            else:
                if policy_actions.dim() > 1:
            ...
            if self.use_gradient_penalty:
                ...
                expert_state_action = torch.cat([expert_obs, expert_actions.reshape(expert_obs.shape[0], -1)], dim=-1)
                policy_state_action = torch.cat([policy_obs, policy_actions.reshape(policy_obs.shape[0], -1)], dim=-1)
                
                # Ensure both tensors are on the same device before gradient penalty
                expert_state_action = expert_state_action.to(self.config.device)
                policy_state_action = policy_state_action.to(self.config.device)
...
def train_gail(args, vecenv):
    ...
    expert_actions = torch.tensor(expert_actions).to(args.train.device).reshape(-1, expert_actions.shape[-1])
    expert_dataset = ExpertDataset(expert_obs, expert_actions, device=args.train.device)
    
    print(f"expert_obs shape: {expert_obs.shape}, expert_actions shape: {expert_actions.shape}")

    # Create discriminator
    obs_dim = expert_obs.shape[-1]
    
    # Determine action space info based on environment action type
    action_space_type = getattr(args.environment, 'action_type', 'discrete')
    
    if action_space_type == "continuous":
        # For continuous actions, use the actual action dimension
        action_dim = expert_actions.shape[-1]
        print(f"Expert data contains continuous actions. action_dim = {action_dim}")
        num_action_classes = None
    else:
        print("Expert data contains discrete actions.")
        # For discrete actions, treat as single dimension
        action_dim = 1
        num_action_classes = int(expert_actions.max().item()) + 1 if expert_actions.numel() > 0 else 91
    
    print(f"Discriminator setup: obs_dim={obs_dim}, action_dim={action_dim}, action_type={action_space_type}")
    ...
    while ppo_data.global_step < args.train.total_timesteps:
        try:
            ...
            if gail_trainer.should_update_discriminator():
                ...
                if hasattr(ppo_data, 'wandb') and ppo_data.wandb:
                    ppo_data.wandb.log({
                        ...
                        'action_specs/average_action': actions_slice.mean().item(),
                        'action_specs/std_action': actions_slice.std().item()
                    })
            ...
            if args.train.visualize and (step_count % args.train.vis_interval == 0 or step_count == 1):
                # Create visualizer
                visualizer = GAILVisualizer(
                    save_dir="irl/vis", 
                    env=vecenv.driver_env if hasattr(vecenv, 'driver_env') else vecenv.env,
                    device=args.train.device
                )
                print(f"\n=== Generating Visualizations at Step {step_count} ===")
                visualizer.visualize_policy_rollout(policy, step_count, args.train.vis_num_rollouts, args.train.vis_rollout_length)
                print(f"=== Visualization Complete for Step {step_count} ===\n")
