...
def train_discriminator(policy_data, expert_data, discriminator, optimizer, minibatch_size=None, test_accuracy=False):
    ...
    # Validate minibatch_size for each dataset
    if minibatch_size is None or minibatch_size <= 0:
        expert_minibatch_size = expert_data.shape[0]
        policy_minibatch_size = policy_data.shape[0]
    else:
        expert_minibatch_size = min(minibatch_size, expert_data.shape[0])
        policy_minibatch_size = min(minibatch_size, policy_data.shape[0])
    
    # Calculate number of minibatches for each dataset
    expert_num_batches = (expert_data.shape[0] + expert_minibatch_size - 1) // expert_minibatch_size
    policy_num_batches = (policy_data.shape[0] + policy_minibatch_size - 1) // policy_minibatch_size
    max_batches = max(expert_num_batches, policy_num_batches)
...
def train(args, vecenv):
    ...
    if(args.gail.use_full_dataset):
        run_name += "_full_dataset"
    else:
        run_name += "_sampled_dataset"
    ...
    while data.global_step < args.train.total_timesteps:
        try:
            ...
            if(args.gail.use_full_dataset):
                ...
                generator_data = obs_data[torch.randperm(obs_data.shape[0])[:args.gail.data_batch_size]]
            ...
            (expert_acc, policy_acc) = train_discriminator(generator_data, expert_data, discriminator, discriminator_optimizer, args.gail.minibatch_size, test_accuracy=test_accuracy)
